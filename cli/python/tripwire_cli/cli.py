...
Commands:
  tripwire login                       log in with an emailed sign-in code,
                                       cache a token
...
@click.option(
    "--email",
    help="email to sign in with (defaults to `git config user.email`)",
)
@click.pass_obj
@_handle_errors
def login(obj: Context, email: str | None) -> None:
    ...
    Sends a 6-digit code to your email (`--email`, else `git config
    user.email`, else prompted) and asks you to enter it. Login is interactive:
    ...
    creds = _email_login(client, server, obj.git_email(), email=email)
...
def whoami(obj: Context) -> None:
    ...
    # Surface the server only when it is a non-default (self-hosted / test) target.
    if creds.server:
